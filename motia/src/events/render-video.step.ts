...
export const handler: Handlers['RenderVideo'] = async (input, { emit, logger }) => {
  const { jobId, concept, quality, manimCode, usedAI, generationType } = input

  logger.info(`Rendering video for job ${jobId}, quality=${quality}`)
  ...
  try {
    ...
    logger.info(`Job ${jobId}: Wrote code to ${codeFile}`)
    ...
    logger.info(`Job ${jobId}: Running manim ${args.join(' ')}`)
    ...
    if (!videoPath) {
      logger.error(`Job ${jobId}: Video file not found in ${mediaDir}`)
      await emit({
        topic: 'video.failed',
        data: {
          jobId,
    ...
    logger.info(`Job ${jobId}: Video saved to ${outputPath}`)
    ...
    logger.info(`Job ${jobId}: Rendering complete`)

  } catch (error) {
    logger.error(`Job ${jobId}: Render failed`, error)
    await emit({
      topic: 'video.failed',
      data: {
        jobId,
    ...
    try {
      ...
      logger.info(`Job ${jobId}: Cleaned up temp directory`)
    } catch {
      logger.warn(`Job ${jobId}: Failed to cleanup temp directory`)
    }
  }
}
